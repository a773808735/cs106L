...
template <typename K, typename M, typename H>
HashMap<K, M, H>::HashMap(const HashMap &map) : _size(0), _hash_function(map._hash_function)
{
    this->_buckets_array.resize(map._buckets_array.size(), nullptr);
    for (const value_type &p : map)
    {
        this->insert(p);
    }
}
